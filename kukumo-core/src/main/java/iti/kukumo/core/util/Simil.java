    ...
    private int compare(Deque<String> baseInputStack, Deque<String> inputStack) {
        ...
        while (windowSize > 0) {
            ...
            while (pos + windowSize - 1 < comp1.length()) {
                ...
                String substring = comp1.substring(pos, pos + windowSize);
                if (comp2.contains(substring)) {

                    // yes, so we take the parts that do not belong to our matching
                    // string and push them onto the stack for later examination
                    String[] comp2Rest = comp2.split(Pattern.quote(substring), 2);
                    String[] comp1Rest = comp1.split(Pattern.quote(substring), 2);
                    ...
                    if (resultLen > 1) {
                        pushToStack(baseInputStack, inputStack, comp2Rest, comp1Rest, resultLen);
                    }
